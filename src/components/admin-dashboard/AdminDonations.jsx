...
const AdminDonations = () => {
  ...
  return (
    <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className={`bg-white ${toggleTheme ? 'dark:bg-gray-800' : ''} rounded-lg shadow`}>
        <div className="p-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className={toggleTheme ? 'bg-gray-700' : 'bg-gray-50'}>
                <tr>
                  {['Donor', 'Campaign', 'Amount', 'Date', 'Status', 'Actions'].map((header) => (
                    <th
                      key={header}
              ...
              <tbody className={`divide-y divide-gray-200 ${toggleTheme ? 'bg-gray-800' : 'bg-white'}`}>
                {donations.map((donation) => (
                  <tr key={donation.id}>
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${toggleTheme ? 'text-gray-200' : 'text-gray-900'}`}>
                      <div className="text-sm">{donation.campaign}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${toggleTheme ? 'text-gray-200' : 'text-gray-900'}`}>
                      <div className="text-sm">{donation.amount}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${toggleTheme ? 'text-gray-400' : 'text-gray-500'}`}>
                      {donation.date}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusStyle(donation.status)}`}>
                        {donation.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
