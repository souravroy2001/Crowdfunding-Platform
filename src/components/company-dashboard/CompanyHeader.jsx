...
const defaultData = {
    ...
    description: "FundHive is a blockchain-based crowdfunding platform that empowers creators to raise funds for their projects.",
    raised: 124580,
    goal: 150000,
    daysLeft: 15,
    supporters: 847,
    ...
    teamMembers: [
        ...
        {
            avatar: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?ixlib=rb-1.2.1&auto=format&fit=facearea&facepad=2&w=32&h=32&q=80",
            name: "David Kim",
            role: "CMO"
        }
    ]
        ...
        <div className={`rounded-xl ${theme ? "bg-white" : "bg-gray-800"} shadow-sm overflow-hidden`}>
            <div className="p-6">
                ...
                <div className="space-y-2">
                    <h1 className={`text-2xl font-bold ${theme ? "text-gray-900" : "text-white"}`}>
                        {companyName}
                    </h1>
                    <h2 className={`text-xl ${theme ? "text-gray-700" : "text-gray-300"}`}>
                        {campaignTitle}
                    </h2>
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="flex items-center space-x-3">
                        <div className={`p-2 rounded-lg ${theme ? "bg-blue-100" : "bg-blue-800/20"}`}>
                            <FaChartLine className={`h-5 w-5 ${theme ? "text-blue-600" : "text-blue-400"}`} />
                        </div>
                        <div>
                            <p className={`text-sm font-medium ${theme ? "text-gray-500" : "text-gray-400"}`}>
                                Average Donation
                            </p>
                            <p className={`text-lg font-semibold ${theme ? "text-gray-900" : "text-white"}`}>
                    ...
                    <div className="flex items-center space-x-3">
                        <div className={`p-2 rounded-lg ${theme ? "bg-green-100" : "bg-green-800/20"}`}>
                            <FaUsers className={`h-5 w-5 ${theme ? "text-green-600" : "text-green-400"}`} />
                        </div>
                        <div>
                            <p className={`text-sm font-medium ${theme ? "text-gray-500" : "text-gray-400"}`}>
                                Total Supporters
                    ...
                    <div className="flex items-center space-x-3">
                        <div className={`p-2 rounded-lg ${theme ? "bg-purple-100" : "bg-purple-800/20"}`}>
                            <FaRegClock className={`h-5 w-5 ${theme ? "text-purple-600" : "text-purple-400"}`} />
                        </div>
                        <div>
                            <p className={`text-sm font-medium ${theme ? "text-gray-500" : "text-gray-400"}`}>
                                Days Left
                ...
                {/* Team Members */}
                <div className="mt-6">
                    <h3 className={`text-sm font-medium ${theme ? "text-gray-700" : "text-gray-300"} mb-3`}>
                        Team Members
                    </h3>
                        ...
                        {teamMembers.map((member, index) => (
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className={`text-xs ${theme ? "text-gray-500" : "text-gray-400"}`}>
                                        {member.role}
                                    </p>
                                </div>
                            </div>
